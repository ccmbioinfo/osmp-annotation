...
def get_unique_variants(report: str) -> str:
    variants = pd.read_csv(report, usecols=["referenceName", "start", "end"]).reindex(columns=["referenceName", "start", "end"]).drop_duplicates()

    variants["range"] = variants.apply(
        lambda row: f"{row.referenceName}:{row.start - 1}-{row.end + 1}", axis=1
    )

    loci = variants.to_string(index=False, header=False, columns=["range"]).replace('\n', ' ').strip()
